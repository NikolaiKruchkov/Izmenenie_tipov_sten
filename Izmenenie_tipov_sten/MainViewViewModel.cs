...
namespace Izmenenie_tipov_sten
{
            ...
            using (var ts = new Transaction(doc, "Set wall type"))
            {
                ...
                foreach (var pickedObject in PickedObjects)
                {
                    if (pickedObject is Wall)
                    {
                        ...
                        Parameter wallTypeParameter = oWall.get_Parameter(BuiltInParameter.ELEM_TYPE_PARAM);
                        wallTypeParameter.Set(SelectedWallTypes.Id);
                    }
                }
                ts.Commit();
            }
